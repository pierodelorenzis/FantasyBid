  ...
  function render({ auction, session, participant }) {
    ...
    const side =
      session.role === "admin"
        ...
        : `<article class="bid-panel"><p>La tua disponibilità</p><h2>${money(participant.budget - (participant.committed || 0))} <small>crediti</small></h2><div class="bar-label"><span>Budget impegnato</span><b>${participant.committed || 0} / ${participant.budget}</b></div><div class="bar"><i style="width:${((participant.committed || 0) / participant.budget) * 100}%"></i></div><hr><label>La tua offerta <small>min. ${money(minimum)}</small></label><div class="offer"><button id="minus">−</button><input id="amount" type="number" min="${minimum}" value="${minimum}"><button id="plus">+</button></div><button id="bid" class="primary" ${auction.canBid ? "" : "disabled"}>Fai la tua offerta <span id="offerAmount">${money(minimum)}</span></button><small class="note">Il server convalida disponibilità, rilanci minimi e tetti per fascia.</small></article>`;

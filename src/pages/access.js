...
export function createAccessPages({ app, stop, saveSession, loadAuction }) {
  function landing() {
    ...
    app.innerHTML = `<main class="landing"><nav class="land-nav"><a class="brand"><span class="brand-mark">F</span><span class="brand-name">Fantasy<span>Bid</span></span></a><button id="showJoin" class="ghost">Entra in un'asta <i data-lucide="arrow-right"></i></button></nav><section class="hero"><p class="eyebrow">L'ASTA DEL FANTACALCIO, SEMPLIFICATA</p><h1>Tutta la tua lega.<br><em>Un'unica asta.</em></h1><p class="lead">Crea una stanza, condividi il link e gestisci ogni rilancio in tempo reale.</p><button class="primary big" id="showCreate">Crea una nuova asta <i data-lucide="arrow-right"></i></button></section><footer class="landing-footer"><div class="features"><span><i data-lucide="circle-check"></i> Nessuna registrazione</span><span><i data-lucide="sliders-horizontal"></i> Regole personalizzabili</span><span><i data-lucide="download"></i> Export CSV incluso</span></div><div class="landing-credit"><span>Made with love by Pedro.</span><a class="tip-link" href="https://www.paypal.me/pierodelorenzis" target="_blank" rel="noopener noreferrer"><i data-lucide="heart"></i> Lascia un Tip</a></div></footer></main>`;
    renderIcons();

...
export function createCatalogFeature() {
  ...
  function wire({ auction: initialAuction, session, setAuction, renderLive }) {
    ...
    const draw = () => {
      const storedTierNames = auction.tierSettings.map((item) => item.name);
      const tierNames = auction.tierSettings

...
function render(page = "live") {
  activePage = page;
  app.innerHTML = `<div class="shell ${mobileMenuOpen ? "menu-open" : ""}">${nav(page)}<section class="content"><header><button id="mobileMenu" class="mobile-menu" aria-label="Apri menu">☰</button><span class="crumb">${auction.name} <b>·</b> Codice <strong>${auction.code}</strong></span>${session.role === "admin" ? '<button id="share" class="ghost">Condividi link</button>' : ""}</header><div class="page">${page === "live" ? live() : page === "team" ? team() : page === "teams" ? teams() : admin()}</div></section></div>`;
  $$(".nav").forEach(
    (x) =>
      (x.onclick = () => {
        mobileMenuOpen = false;
...
function team() {
  let m = me(),
    players = m.players || [];
  return `<div class="title-row"><div><p class="eyebrow">${session.name.toUpperCase()}</p><h1>La mia <em>rosa.</em></h1></div><button id="exportTeam" class="ghost">↓ Esporta CSV</button></div><div class="stats"><div><small>CREDITI RESIDUI</small><b>${money(m.budget - (m.committed || 0))}</b></div><div><small>GIOCATORI ACQUISTATI</small><b>${players.length} <i>/ ${auction.totalSlots}</i></b></div><div><small>CREDITI SPESI</small><b>${money(m.committed || 0)}</b></div></div><div class="table"><div class="thead"><span>GIOCATORE</span><span>RUOLO</span><span>SQUADRA</span><span>ACQUISTO</span></div>${players.map((p) => `<div><b>${p.name}</b><span>${p.role}</span><span>${p.team}</span><em>${money(p.price)}</em></div>`).join("") || '<p class="muted pad">Nessun giocatore acquistato.</p>'}</div>`;
}
function teams() {

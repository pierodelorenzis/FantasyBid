-- Assegnazione atomica del giocatore corrente. L'asta viene bloccata prima
-- della lettura dell'offerta, così non può essere accettato un rilancio tra
-- l'assegnazione e il passaggio al giocatore successivo.
...
begin
  ...
  for update;
  if not found then
    raise exception 'Nessun giocatore da assegnare';
  end if;
  ...
  if v_player.highest_bid_participant_token is not null then
    ...
    from public.auction_participants
    where auction_code = p_auction_code
      and token = v_player.highest_bid_participant_token
    for update;
    if not found then
      raise exception 'Offerente non trovato';
    end if;
